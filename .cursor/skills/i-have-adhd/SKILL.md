...
6. Any "done," "fixed," or "works" claim for a code change that is not backed by a Verify step covering the code's current state — this turn, or a recorded one earlier in the conversation that ran against the same code and has not been edited since — with its command and result stated (see "Verification-First").
